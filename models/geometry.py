...
from models.module import ConvBnReLU3D
...
class GeoFeatureFusion(nn.Module):
    def __init__(self, convolutional_layer_encoding="z", mask_type="basic", add_origin_feat_flag=True):
        ...
        # 几种不同的将几何信息给编码进卷积层中，标准卷积、额外增加uv两层再进行卷积、只增加Z一层进行卷积、增加xyz3层再进行卷积
        ...
        # TransposeConvloution: out = (in-1)*stride - 2*padding + kernel + outpadding
        # layer4: 变为2倍  layer2: 变为2倍  layer0: 不变  layer: 变为2倍   output: 不变
        ...
        # # layer3: 变为2倍  layer4: 不变  layer5: 变为2倍  layer6: 不变   layer7: 变为2倍
        ...
        # output
        # stage1: 变为2倍  stage2: 变为2倍  stage3: 不变
    ...
    def forward(self, rgb, depth, confidence, depth_values, stage_idx, origin_feat, intrinsics_matrices_stage):
        ...
        d = (depth - depth_min) / (depth_max - depth_min)  # 归一化
        ...
        # -----------------------------------------------------------------------------------------
        # Note: 这个中间过程中的图像特征size大小（H,W）变化都是相当于这个一开始input的大小而言的,
        # 所以再经过一系列变化之后，最后的输出size还是要和原输入input一样大小才行
        ...
        #     if stage_idx == 1:
        #         rgbdepth_feature = self.rgbdepth_decoder_stage1(fusion5) # (b,32,h,w)
        ...
        #     if stage_idx == 2:
        #         rgbdepth_feature = self.rgbdepth_decoder_stage2(fusion6) # (b,16,h,w)
        #         if self.add_origin_feat_flag:
        ...
        #     if stage_idx == 3:
        #         rgbdepth_feature = self.rgbdepth_decoder_stage3(fusion7) # (b,8,h,w)
        #         if self.add_origin_feat_flag:
        ...
        if stage_idx >= 1: 
            ...
            if stage_idx == 1:
                ...
                if self.add_origin_feat_flag:
                    ...
                    final_feature = self.final_decoder_stage2(rgbdepth_feature)
                # print("*************&&&&&&&&&")
                # print("Stage2: the shape of final_feature is {}".format(final_feature.shape))  # torch.Size([8, 16, 256, 320])
        ...
        if stage_idx >= 2:
            ...
            if stage_idx == 2:
                ...
                if self.add_origin_feat_flag:
                    ...
                    final_feature = self.final_decoder_stage3(rgbdepth_feature)
                # print("*************&&&&&&&&&")
                # print("Stage3: the shape of final_feature is {}".format(final_feature.shape))   # torch.Size([8, 8, 512, 640])
...
class GeoRegNet2d(nn.Module):
    def __init__(self, input_channel=32, base_channel=8, convolutional_layer_encoding="std"):
        ...
        self.depthpooling = nn.MaxPool3d((2,1,1),(2,1,1))  # 只对dim=2上的depth通道进行降采样一半 （kernel=2,stride=2）
        self.basicpooling = nn.MaxPool3d((1,2,2), (1,2,2))  # 只对H/W进行降采样一半
    ...
    def forward(self, x, stage_idx, geo_reg_data=None):
        ...
        if self.convolutional_layer_encoding == "std":
            ...
            # if stage_idx == 2:  # GeoMVSNet所用代码，但是其depth_hyp为(8,8,4,4)
            #     print("$$$$$$$$$ it is stage3")
            #     geo_s1 = self.depthpooling(prob_volume)  # 沿着depth方向进行降采样，为了后面与geo进行cat。在depth上的通道数能够对齐
            # else:
            #     print("the shape of prob_volume,before depthpooling is: ", prob_volume.shape)  # torch.Size([4, 1, 48, 256, 320])
            #     geo_s1 = prob_volume   # B 1 D H W
            #     geo_stemp = self.depthpooling(prob_volume)
            #     print("^^^^^^^^^@@@@@ the shape of prob_volume， after depthpooling, is: ", geo_stemp.shape) # torch.Size([4, 1, 24, 256, 320])
            # geo_s2 = self.basicpooling(geo_s1)
            # print("the shape of geo_s2， after depthpooling, is: ", geo_s2.shape)  # torch.Size([4, 1, 48, 128, 160])
            # geo_s3 = self.basicpooling(geo_s2)
            # print("the shape of geo_s3， after depthpooling, is: ", geo_s3.shape)  # torch.Size([4, 1, 48, 64, 80])

            if stage_idx == 1:  # stage2, 改了假设深度数量,由(48,32,8)变为（64,32,8）
                geo_s1 = self.depthpooling(prob_volume)   # 从64变为32，采取了一次池化操作
            else:  # stage3
                geo_s1_pre = self.depthpooling(prob_volume)
                geo_s1 = self.depthpooling(geo_s1_pre)  # 因为要从32变为8,所以采取了两次池化操作
            geo_s2 = self.basicpooling(geo_s1)   
            geo_s3 = self.basicpooling(geo_s2) 
  
        feature = self.conv_init(x)     # B 8 D H W 这里将dim=1上的通道统一都变成了8通道，之后再将cost_volume与prob_volume进行cat拼接
...
class Reg_BasicBlockGeo(nn.Module):
    ...
    def forward(self, x, g1=None, g2=None):
        ...
        if g1 is not None:
            # print("*************")
            # print(" x cost_volume shape is: ", x.shape)   # torch.Size([4, 8, 32, 256, 320]),之所以dim=1的通道数是8，因为是一开始就经过处理了
            # print(" g1 prob_volume shape is: ", g1.shape)  # torch.Size([4, 1, 48, 256, 320])
            # 原因错在prob_volume，cost_volume是没有问题的。因为stage2的prob_volume是从stage1直接上采样过来的。
            # 而这里连接cat操作是在stage2之后，而stage2的cost volume的Ndepth变为了32，不在是stage1的48
            # 说明前面针对prob_volume的F.interpolate操作只是在最后两个维度上进行操作了。
            x = torch.cat((x, g1), 1)
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
