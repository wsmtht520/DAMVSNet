...
class DepthNet(nn.Module):
    ...
    def forward(self, stage_idx, features, proj_matrices, depth_values, num_depth, regnet, geo_reg_data=None):
        ...
        ## prob_volume = F.softmax(prob_volume_pre, dim=1)  ## 将深度信息压缩为0~1之间的分布，得到概率体
         ...
         # @Note: cost regularization  for geo_mvsnet
        if geo_reg_data == {}:
            # basic  stage0
            cost_reg = regnet(volume_agg)
        else: # Note: geo_mvsnet的cost_reg过程中已经squeeze(1),所以后面不再额外进行squeeze(1)
            # probability volume geometry embedding  stage1 and stage2
            cost_reg = regnet(volume_agg, stage_idx, geo_reg_data)
        ...
        return {"depth": depth,  "photometric_confidence": photometric_confidence, 'variance': exp_variance, "prob_volume": prob_volume}
...
class CascadeMVSNet(nn.Module):
    def __init__(self, refine=False, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1], share_cr=False,
        ...
        # embedding prob_volume的init for geomvsnet
        self.RegNet_stages = nn.ModuleList()
        self.group_cor_dim_stages = [32, 16, 8]  # self.feature.out_channels
        # print("the feature.out_channels is: ", self.feature.out_channels)  #[32, 16, 8] 
        ...
        for stage_idx in range(self.num_stage):
            ...
            if self.geo_reg_flag:  # 可以看出Geo_MVSNet4阶段都是调用GeoRegNet2d,而不是Reg2d
                self.RegNet_stages.append(GeoRegNet2d(input_channel=in_dim, base_channel=self.reg_base_channel,
                                                      convolutional_layer_encoding=self.geo_reg_encodings[stage_idx]))
            else:  # basice: input_channel也是（8,8,4,4）
                self.RegNet_stages.append(Reg2d(input_channel=in_dim, base_channel=self.reg_base_channel))
    ...
    def forward(self, imgs, proj_matrices, depth_values,intrinsics_matrices):
        ...
        # print("***********&&&&&&&&&&")
        # print("the size of img is: {}".format(imgs.shape))   # torch.Size([7, 5, 3, 512, 640])
        # step 1. feature extraction
        features = []
        ...
        for stage_idx in range(self.num_stage):
            ...
            # @Note cost regularization for embedding pro_volume
            geo_reg_data = {}
            if self.geo_reg_flag:
                geo_reg_data['depth_values'] = depth_values  # 这个是在stage0时候，让geo_reg_data有值,之后都调用GeoRegNet2d
                if stage_idx >= 1 and self.geo_reg_encodings[stage_idx] == 'z':
                    # print(" prob_volueme_last shape is: ", prob_volume_last.shape)  # torch.Size([4, 48, 128, 160])
                    prob_volume_last = F.interpolate(prob_volume_last, size=None, scale_factor=2, mode="bilinear", align_corners=False)
                    # print(" prob_volueme_interpolate shape is: ", prob_volume_last.shape)  # torch.Size([4, 48, 256, 320])
                    geo_reg_data["prob_volume_last"] = prob_volume_last

            # geomvsnet: embedding prob_volume for cost_volume regularization
            outputs_stage = self.DepthNet(stage_idx, features_stage, proj_matrices_stage,
                                          depth_values=F.interpolate(depth_range_samples.unsqueeze(1),
                                                                     [self.ndepths[stage_idx], img.shape[2]//int(stage_scale), img.shape[3]//int(stage_scale)],
                                                                     mode='trilinear',
