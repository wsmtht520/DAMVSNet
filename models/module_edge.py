...
# def enconvbnrelu(in_channels, out_channels, kernel_size=3, stride=1, padding=1, output_padding=1):
...
# def enconvbnrelu(in_channels, out_channels, kernel_size=3, stride=1, padding=1):
#     return nn.Sequential(
...
# class enconvbnrelu(nn.Module):
#     def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
#         super(enconvbnrelu, self).__init__()
...
# class Edge_FeatureNet(nn.Module):
#     def __init__(self, base_channels, num_stage=3, stride=4, arch_mode="fpn"):
#         super(Edge_FeatureNet, self).__init__()
...
# fix: fusion edge_feature and normal_feature
# class EdgeFeatureFusion(nn.Module):
#     def __init__(self, mode = "plus"):
#         super(EdgeFeatureFusion,self).__init__()
#         self.mode = mode
#         # size不变
#         self.fusion_encoder_stage1 = enconvbnrelu(in_channels=32, out_channels=32, kernel_size=3, stride=1, padding=1)
#         self.fusion_encoder_stage2 = enconvbnrelu(in_channels=16, out_channels=16, kernel_size=3, stride=1, padding=1)
#         self.fusion_encoder_stage3 = enconvbnrelu(in_channels=8, out_channels=8, kernel_size=3, stride=1, padding=1)


#     def forward(self, feat_stage, edge_feat_stage, stage_idx):
#         fusion_feature = []
#         for surface_feat_stage, edges_feat_stage in zip(feat_stage, edge_feat_stage):
#             fuse_feat = surface_feat_stage + edges_feat_stage  # 依次对list中的tensor进行相加融合操作
#             # print("the type of fuse_feat is: ", type(fuse_feat))  # the type of fuse_feat is:  <class 'torch.Tensor'>
#             # print(fuse_feat.shape)    # torch.Size([5, 32, 128, 160])
#             if stage_idx == 0:
#                 fusion_feat = self.fusion_encoder_stage1(fuse_feat)
#             elif stage_idx == 1:
#                 fusion_feat = self.fusion_encoder_stage2(fuse_feat)
#             elif stage_idx == 2:
#                 fusion_feat = self.fusion_encoder_stage3(fuse_feat)
#             fusion_feature.append(fusion_feat)
