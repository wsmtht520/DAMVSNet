...
class DepthNet(nn.Module):
    ...
    def forward(self, stage_idx, features, proj_matrices, depth_values, num_depth, cost_regularization, prob_volume_init=None):
        ...
        # print("$$$$$$$$$$$$$$$$   the num of imgs is: {}".format(num_views))  # 5
        ...
        cost_reg = cost_regularization(volume_agg)
...
class CascadeMVSNet(nn.Module):
    def __init__(self, refine=False, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1], share_cr=False,
        ...
        # self.edge_feature = Edge_FeatureNet(base_channels=8, stride=4, num_stage=self.num_stage, arch_mode=self.arch_mode)
        # cost volume aggregation
        # 3 stage,feature.out_channels: [32,16,8]
        # self.cost_aggregation = CostAgg(agg_mode, self.feature.out_channels)

        # self.EdgeFeatureFusionNet = EdgeFeatureFusion(mode="plus")
    ...
    def forward(self, imgs, proj_matrices, depth_values, edge_imgs,intrinsics_matrices):
        ...
        for nview_idx in range(imgs.size(1)):  #imgs shape (B, N, C, H, W)
            ...
            edge = edge_imgs[:, nview_idx]
            # features.append(self.feature(img))
            # edge_features.append(self.edge_feature(edge))
        ...
        for stage_idx in range(self.num_stage):
            ...
            # surface_features_stage = [feat[stage_name] for feat in features]
            # edge_features_stage = [edge_feat[stage_name] for edge_feat in edge_features]
            # print("&&&&&&&")
            # print(surface_features_stage[0].shape)  # torch.Size([5, 32, 128, 160])  batchsize=5,N=5
            # print(len(surface_features_stage))  # 5
            # print(type(surface_features_stage))  # <class 'list'>
            # print(edge_features_stage[0].shape)   # torch.Size([5, 32, 128, 160])
            # print(len(edge_features_stage))  # 5
            # print(type(edge_features_stage))   # <class 'list'>

            proj_matrices_stage = proj_matrices["stage{}".format(stage_idx + 1)]
            stage_scale = self.stage_infos["stage{}".format(stage_idx + 1)]["scale"]
            intrinsics_matrices_stage = intrinsics_matrices[stage_name]

            # fix: fused edge_feature
            # 注意：这里传入的是list，真正处理的时候要是tensor类型，否则会error
            # 最后处理完毕也要是list,里面包含N个tensor
            # fuse_feature = [edge_f+sur_f for edge_f, sur_f in zip(surface_features_stage,edge_features_stage)]
            # features_stage = self.EdgeFeatureFusionNet(surface_features_stage, edge_features_stage, stage_idx)
            
            # print("******")
            # print(type(features_stage))   # <class 'list'>
            # print(features_stage[0].shape)  # torch.Size([5, 32, 128, 160])
            # print(len(features_stage))   # 5
