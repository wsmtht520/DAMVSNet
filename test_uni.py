...
from filter.pcd import pcd_filter
from filter.dypcd import dypcd_filter
# from filter.dypcd import *
...
parser.add_argument('--dataset', default='generla_eval', help='select dataset')
...
# parser.add_argument('--outdir', default='/home/tianhaitao/repo/cascade-mvsnet/CasMVSNet/outputs3', help='output dir')
# parser.add_argument('--outdir', default='./outputs_sample22', help='output dir')  # 添加深度采样模块 3 sigma和深度图融合参数
# parser.add_argument('--outdir', default='./outputs_sample222', help='output dir')  # 添加深度采样模块 1.5 sigma和深度图融合参数
# parser.add_argument('--outdir', default='./outputs_ndepths64_1103_numviewis4', help='output dir')  # 添加深度采样模块 自适应深度间隔
# parser.add_argument('--outdir', default='./outputs_adapt1030', help='output dir') # 添加深度采样模块 自适应深度间隔 视图数为4
# parser.add_argument('--outdir', default='./outputs_test1119_cpcloss', help='output dir')  # (0.3,0.25,4)
# parser.add_argument('--outdir', default='./outputs_test1120_cpcloss', help='output dir')  # 修改了后融合参数：由0.3,0.25,4 -> 0.7,0.25,3
# parser.add_argument('--outdir', default='./outputs_test1121_cpcloss', help='output dir')  # 修改了后融合参数：由0.3,0.25,4 -> 0.8,0.25,3
# parser.add_argument('--outdir', default='./outputs_test1122_cpcloss', help='output dir')   # (0.7,0.25,4)
# parser.add_argument('--outdir', default='./outputs_test1225_depthprior', help='output dir')  #  (0.3,0.25,4)
# parser.add_argument('--outdir', default='./outputs_test1226_depthprior', help='output dir')  # (0.5,0.25,3) 
# parser.add_argument('--outdir', default='./outputs_test1227_depthprior', help='output dir')  # (0.4,0.25,3) 
# parser.add_argument('--outdir', default='./outputs_test1229_depthprior_N5', help='output dir')  # N=5, (0.4,0.25,3) 
# parser.add_argument('--outdir', default='./outputs_test1230_depthprior_N5', help='output dir')  # N=5, (0.3,0.25,4) 
# parser.add_argument('--outdir', default='./outputs_test1231_depthprior_N5', help='output dir')  # N=5, (0.6,0.25,3) 
# parser.add_argument('--outdir', default='./outputs_test1231_depthpriorN3_adaptCost', help='output dir')  # N=3,costvolume adaptive (0.3,0.25,4)
# parser.add_argument('--outdir', default='./outputs_test0101_depthpriorN3_adaptCost', help='output dir')  # N=3,costvolume adaptive (0.4,0.25,3)
# parser.add_argument('--outdir', default='./outputs_test0102_depthpriorN3_adaptCost', help='output dir')  # N=3,costvolume adaptive (0.7,0.25,3)  
# parser.add_argument('--outdir', default='./outputs_test0103_depthpriorN3_adaptCost', help='output dir')  # N=3,costvolume adaptive (0.7,0.25,4) 
# parser.add_argument('--outdir', default='./outputs_test0103_depthpriorN5_adaptCost', help='output dir')  # N=5,costvolume adaptive (0.3,0.25,4) 
# parser.add_argument('--outdir', default='./outputs_test0106_depthpriorN5_adaptCost', help='output dir')  # N=5,costvolume adaptive (0.4,0.25,3)   
# parser.add_argument('--outdir', default='./outputs_test0318_depthpriorN5_adaptCost_64', help='output dir')  # N=5,costvolume adaptive (0.1,0.2,3), (64,32,8)   1600*1152 
# parser.add_argument('--outdir', default='./outputs_test0319_depthpriorN5_adaptCost_64', help='output dir')  # N=5,costvolume adaptive (0.1,0.15,3), (64,32,8)   1152*864    
# parser.add_argument('--outdir', default='./outputs_test0321_depthpriorN5_nocpc_adaptCost_64', help='output dir') # 无图像合成损失
parser.add_argument('--outdir', default='./outputs_test04193NoTrans_tnteval_tnt/inter', help='output dir')    
# parser.add_argument('--outdir', default='./outputs_test0117_depthpriorN5_adaptCost_edge', help='output dir')  # N=5,costvolume adaptive (0.3,0.25,4), edge        
parser.add_argument('--display', action='store_true', help='display depth images and masks')

parser.add_argument('--share_cr', action='store_true', help='whether share the cost volume regularization')

# parser.add_argument('--ndepths', type=str, default="48,32,8", help='ndepths')
parser.add_argument('--ndepths', type=str, default="64,32,8", help='ndepths')  # 由（48,32,8）改为（64,32,8）
parser.add_argument('--depth_inter_r', type=str, default="4,2,1", help='depth_intervals_ratio')
parser.add_argument('--cr_base_chs', type=str, default="8,8,8", help='cost regularization base channels')
parser.add_argument('--grad_method', type=str, default="detach", choices=["detach", "undetach"], help='grad method')

# parser.add_argument('--interval_scale', type=float, default="1.06", required=True, help='the depth interval scale') # 必选参数
...
# parser.add_argument('--max_h', type=int, default=1152, help='testing max h')
...
# dypcd, from unimvsnet/geomvsnet.   check_geometric_consistency()中对这两个参数有进行处理
parser.add_argument('--dist_base', type=float, default=1 / 4)
parser.add_argument('--rel_diff_base', type=float, default=1 / 1300)

#filter by gimupa
parser.add_argument('--fusibile_exe_path', type=str, default='./fusibile/build2/fusibile')
# parser.add_argument('--prob_threshold', type=float, default='0.9')
parser.add_argument('--prob_threshold', type=float, default='0.1')  # before 设置为0.3
parser.add_argument('--disp_threshold', type=float, default='0.15')  # before 设置为0.25
parser.add_argument('--num_consistent', type=float, default='3')
# parser.add_argument('--num_consistent', type=float, default='3')  # cascade-mvsnet的视图数量设置为4
...
def save_scene_depth(testlist):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            # outputs: (B,H,W)
            outputs = model(sample_cuda["imgs"], sample_cuda["proj_matrices"], sample_cuda["depth_values"], sample_cuda["intrinsics_matrices"])
            # outputs = model(sample_cuda["imgs"], sample_cuda["proj_matrices"], sample_cuda["depth_values"],sample_cuda["edge_imgs"] ,sample_cuda["intrinsics_matrices"])
            end_time = time.time()
            outputs = tensor2numpy(outputs)
            del sample_cuda
            # general_eval.py的输出： "filename": scan + '/{}/' + '{:0>8}'.format(view_ids[0]) + "{}"
            filenames = sample["filename"]
            cams = sample["proj_matrices"]["stage{}".format(num_stage)].numpy()
            imgs = sample["imgs"].numpy()
            # print('Iter {}/{}, Time:{} Res:{}'.format(batch_idx, len(TestImgLoader), end_time - start_time, imgs[0].shape))

            # save depth maps and confidence maps.   3 stage
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src):
    print(" &&&&&^^^^^ it is using the mainfunction of check_geometric")
    width, height = depth_ref.shape[1], depth_ref.shape[0]
    x_ref, y_ref = np.meshgrid(np.arange(0, width), np.arange(0, height))
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    print("*****&&&&& it useing of the function of filter_depth....")
    # the pair file
    ...
    print("******&&&&& it is using of the main funciotn of filter_depth")
...
if __name__ == '__main__':
    ...
    elif args.filter_method == "dypcd":
        print("&&&& begin fusion of dypcd: ", flush=True)
        # dypcd_filter(args, testlist, 1)
        dypcd_filter(args, testlist)
    else:
        gipuma_filter(testlist, args.outdir, args.prob_threshold, args.disp_threshold, args.num_consistent,
                       args.fusibile_exe_path)
    print("**** it is all *****")    

   # 对于dypcd融合函数, unimvsnet、tansmvsnet、geomvsnet大致相同，只不过一些输入和输出以及photo_mask阈值的处理有差异
   # 1、transmvsnet: 读取最终阶段的confidence，然后与一个固定阈值的photo_threshold进行比较
   # confidence = read_pfm(os.path.join(scan_folder, 'confidence/{:0>8}.pfm'.format(ref_view)))[0]
   # photo_mask = confidence > photo_threshold (缺点，所有场景都用的是一样的photo_threshold)

   # 2、unimvsnet: 读取三阶段的confidence，然后用逻辑与np.logical_and处理来得到photo_mask
